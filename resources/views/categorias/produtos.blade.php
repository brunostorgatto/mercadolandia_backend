<x-app-layout>
    ...
    @if (session('success'))
        ...
        <script>setTimeout(() => { const t = document.getElementById('toast-ok'); if(t) t.style.opacity = '0'; }, 3500);</script>
    @endif
                        ...
                        <div class="relative aspect-square bg-slate-100 overflow-hidden">
                            ...
                            @else
                                <div class="w-full h-full flex flex-col items-center justify-center text-slate-300">
                                    ...
                                    <span class="text-xs mt-1 font-medium">Sem foto</span>
                                </div>
                            @endif
                            ...
                            <div class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-2">
                                <button type="button"
                                        ...
                                        data-produto='{{ json_encode(["id" => $produto->id, "nome" => $produto->nome, "preco" => $produto->preco, "unidade_medida" => $produto->unidade_medida, "incremento" => $produto->incremento]) }}'
        ...
        <div class="bg-white rounded-3xl w-full max-w-lg shadow-2xl border border-slate-100 flex flex-col max-h-[95vh] overflow-y-auto">
            ...
            <form id="form-produto" method="POST" action="{{ route('produtos.store', $categoria) }}" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <div id="zona-upload"
                         class="relative border-2 border-dashed border-slate-300 hover:border-emerald-400 rounded-2xl transition-colors cursor-pointer overflow-hidden"
                         style="min-height: 160px;">

                        {{-- Estado vazio --}}
                        <div id="upload-placeholder" class="flex flex-col items-center justify-center gap-2 py-10 text-slate-400">
                            <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                            </svg>
                            <p class="text-sm font-semibold">Clique ou arraste uma foto</p>
                            <p class="text-xs">JPG, PNG, WEBP — Recorte quadrado 1:1</p>
                        </div>

                        {{-- Preview da imagem recortada --}}
                        <img id="preview-recortada" src="" alt="Preview" class="hidden w-full h-48 object-cover rounded-2xl" />

                        <input type="file" id="input-foto" accept="image/*" class="absolute inset-0 opacity-0 cursor-pointer w-full h-full" />
                    </div>

                    {{-- Botão para reselecionar --}}
                    <button type="button" id="btn-trocar-foto" class="hidden mt-2 text-xs font-semibold text-emerald-600 hover:underline">
                        🔄 Trocar foto
                    </button>

                    {{-- Campo oculto que recebe a imagem recortada em base64 --}}
    ...
    <script>
        ...
        function aplicarMascaraMoeda(input) {
            input.addEventListener('input', function () {
                ...
                if (!digits) { this.value = ''; document.getElementById('produto-preco').value = ''; return; }
                ...
                // Atualiza o hidden com valor decimal (ponto como separador para o backend)
        ...
        function abrirModal(modo = 'criar', produto = null) {
            ...
            document.getElementById('preview-recortada').classList.add('hidden');
            document.getElementById('upload-placeholder').classList.remove('hidden');
            document.getElementById('btn-trocar-foto').classList.add('hidden');
            ...
            if (modo === 'criar') {
                ...
                // Preenche display formatado e hidden com valor numérico
        ...
        // Aplica máscara no campo de display
        ...
        modalProduto.addEventListener('click', e => { if (e.target === modalProduto) fecharModal(); });

        // Botões Editar nos cards
        ...
        document.getElementById('btn-confirmar-crop').addEventListener('click', function () {
            ...
            document.getElementById('btn-trocar-foto').classList.remove('hidden');
